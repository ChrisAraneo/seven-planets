...
import { setAiDifficulty, setSimMode, simulateGameWithPersonalities } from './engine'
...
function asciiTable(headers: string[], aligns: ('l' | 'r')[], rows: string[][]): string {
  ...
  const pad = (s: string, w: number, a: 'l' | 'r') => (a === 'r' ? s.padStart(w) : s.padEnd(w))
...
function buildConsoleReport(stats: DiffStat[], sum: Summary): string {
  ...
  L.push('  skilled human. Every difficulty pits it against 6 mastermind rivals')
  L.push('  weakened by that difficulty and its kamikazes. Higher Win% = easier.')
  L.push('')
  ...
  L.push(`  Seats per game ........... ${SEATS} (seat 0 = human proxy, standard)`)
  L.push(`  Wall-clock time .......... ${fmt(elapsed)}s (${(totalGames / elapsed).toFixed(0)} games/s)`)
  L.push('')
  L.push(`  Baseline: with ${SEATS} equal seats, a no-edge player wins ≈ ${fmt(100 / SEATS)}%.`)
  L.push('')
  L.push('  HUMAN-PROXY WIN RATE BY DIFFICULTY (turns cover the proxy\'s wins only)')
  L.push('')

  const headers = ['Difficulty', 'Kamikaze', 'Games', 'Wins', 'Win%', 'AvgT', 'MedT']
...
function buildMarkdownReport(stats: DiffStat[], sum: Summary): string {
  ...
  L.push(`- **Difficulties:** ${stats.length}`)
  L.push(`- **Total games:** ${totalGames.toLocaleString('en-US')}`)
  L.push(`- **Seats per game:** ${SEATS} (seat 0 = human proxy, standard strength)`)
  L.push(`- **Wall-clock time:** ${fmt(elapsed)}s (${(totalGames / elapsed).toFixed(0)} games/s)`)
  L.push('')
  L.push(`> Baseline: with ${SEATS} equal seats, a no-edge player wins ≈ ${fmt(100 / SEATS)}% of games.`)
  L.push('')
  L.push('## Human-proxy win rate by difficulty')
  L.push('')
  L.push('"Turns to win" covers only the human proxy’s victories.')
  L.push('')
  L.push('| Difficulty | Kamikaze | Games | Wins | Win rate | Avg turns | Median turns |')
  L.push('| :--- | ---: | ---: | ---: | ---: | ---: | ---: |')
