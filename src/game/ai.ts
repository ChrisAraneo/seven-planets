...
function rocketCap(pl: Planet): number {
  ...
  return lvl >= 3 ? Infinity : BASE_ROCKET_CAP * Math.pow(2, lvl)
}
...
export function battleWinProb(attackBase: number, defenseBase: number): number {
  ...
  let wins = 0
  for (let a = 0; a <= aR; a++) {
    for (let d = 0; d <= dR; d++) if (attackBase + a > defenseBase + d) wins++
  }
...
function incomePerTurn(s: GameState, p: Player): Cost {
  ...
  for (const pl of owned(s, p)) {
    for (const b of BUILD_ORDER) {
      ...
      if (res && pl.buildings[b]) inc[res] = (inc[res] || 0) + incomeAmount(b, pl.buildings[b])
    }
  }
  return inc
}
...
function expectedActionCopies(s: GameState, t: ActionType): number {
  if (s.turn < ACTION_CARDS_FROM_TURN) return 0
  const types: ActionType[] = ['ATTACK', 'RECRUIT', 'TRADE']
  if (s.turn >= MOVE_CARDS_FROM_TURN) types.push('MOVE')
  if (!types.includes(t)) return 0
...
function influenceIncome(s: GameState, p: Player): number {
  let inc = 0
  for (const pl of owned(s, p)) {
    if ((pl.buildings.EMBASSY || 0) >= 2) inc += 1
    if (p.pacifistStatus) inc += PACIFIST_INFLUENCE
  }
  if (hasB(s, p, 'EMBASSY')) inc += 0.25
  return inc
}
...
export function playerStrength(s: GameState, p: Player): number {
  ...
  const income = owned(s, p).reduce(
    (sum, pl) => sum + BUILD_ORDER.filter((b) => pl.buildings[b] && BUILDINGS[b].income).length * 3,
    0,
  )
...
const AGGRESSIVE = new Set(['aggressor', 'militarist', 'blitzer', 'expansionist', 'opportunist'])
const DEFENSIVE = new Set(['fortifier', 'trader', 'rusher', 'hoarder', 'economist', 'builder'])
...
function aggression(p: Player): number {
  if (p.pacifistStatus || p.personality === 'pacifist') return 0
  if (AGGRESSIVE.has(p.personality)) return W.willAggressive
  if (DEFENSIVE.has(p.personality)) return W.willDefensive
...
export function recruitRate(s: GameState, p: Player): number {
  let bestYield = 0
  for (const pl of owned(s, p)) bestYield = Math.max(bestYield, recruitYieldOf(pl))
  if (!bestYield) return 0
  const oreFlow = (incomePerTurn(s, p).ORE || 0) + (p.hand.ORE || 0) / 4
  const cardFlow = (p.hand.RECRUIT || 0) > 0 ? 0.9 : actionDrawProb(s, 'RECRUIT')
...
function projectedStrike(
  ...
  for (const pl of owned(s, r)) {
    if (pl.id === excludePlanetId || !pl.buildings.SILO) continue
    const n = Math.min(rocketCap(pl), Math.floor(pl.troops + growth) - 1)
    if (n > best.n) best = { n, bonus: siloBonus(pl) }
  }
  return best
}
...
export function holdProbability(
  ...
  for (const r of alive(s)) {
    ...
    for (let t = 1; t <= horizon; t++) {
      ...
      if (strike.n < 2 || strike.n < minTroopsToConquer(g)) continue
    ...
    const pCard =
      (r.hand.ATTACK || 0) > 0 ? 0.95 : 1 - Math.pow(1 - actionDrawProb(s, 'ATTACK'), window)
...
export function immediateFallProb(s: GameState, ownerP: Player, planet: Planet): number {
  if (underTruce(s, planet)) return 0
  let pSafe = 1
  for (const r of alive(s)) {
...
export function planetValue(s: GameState, pl: Planet): number {
  ...
  for (const b of BUILD_ORDER) {
    ...
    if (inc) v += incomeAmount(b, lvl) * CARDS[inc].value * 3
  }
  ...
  for (const target of s.planets) {
    ...
    for (const source of owned(s, p)) {
      ...
      if (maxN >= nConq) {
        ...
        let lean = nConq
        while (lean < maxN && battleWinProb(attackBaseOf(lean, source), def) < minWin) lean++
        ...
        for (const n of new Set([lean, Math.ceil((lean + maxN) / 2), maxN])) {
          ...
          const hold = holdProbability(s, p, target, surv, s.turn + CONQUEST_TRUCE)
...
function nextLevelAllowed(s: GameState, p: Player, planet: Planet, id: BuildingType): number {
...
export function buildingWorth(
  ...
  if (inc) {
    ...
    gross += (incomeAmount(id, level) - incomeAmount(id, level - 1)) * CARDS[inc].value * H * liquidity
  }
  switch (id) {
    case 'BARRACKS': {
      // Recruiting is existential — without it there is no defense and no army.
      if (!hasB(s, p, 'BARRACKS')) gross += H + 6
      else if (level === 1) gross += 3 // a local recruiting ground on another planet
      else {
        // Higher levels multiply recruit throughput (troops per action card).
        const delta =
          recruitYieldOf({ ...planet, buildings: { BARRACKS: level } }) - recruitYieldOf(planet)
        gross += delta * H * 0.5
      }
      if (planet.buildings.SILO) gross += 3 // recruits muster where the rockets are
      break
    }
    case 'SILO': {
      if (p.pacifistStatus) break // can never attack again — worthless
      // Conquest is the ONLY win condition; the first silo grows more urgent
      // as the game matures and rivals arm up.
      gross += !hasB(s, p, 'SILO') ? H * 0.7 + s.turn / 10 : 2 + SILO_HIT_BONUS * 0.8 + level
      if (planet.buildings.BARRACKS) gross += 3 // co-locate with the recruiting ground
      ...
      if (hasB(s, p, 'SILO')) {
        ...
        for (const tp of s.planets) {
          if (tp.ownerId === p.id || !s.players[tp.ownerId].alive || underTruce(s, tp)) continue
          minNeed = Math.min(minNeed, minTroopsToConquer(tp.troops))
        }
        const newCap =
          level >= 3 ? Infinity : BASE_ROCKET_CAP * Math.pow(2, level)
        if (minNeed !== Infinity && rocketCap(planet) < minNeed && newCap >= minNeed) gross += 7
      }
      break
    }
    case 'SHIELD': {
...
function cardAppearProb(s: GameState, id: BuildingType, withinTurns: number): number {
  const eligible = BUILD_ORDER.filter((b) =>
    b === 'LAB' ? s.turn >= ADVANCED_FROM_TURN : b === 'SINGULARITY' ? singularityLive(s) : true,
  )
  if (!eligible.includes(id)) return 0.15 // could unlock later — small hope
  const per = Math.min(1, 5 / eligible.length)
  return 1 - Math.pow(1 - per, Math.max(1, withinTurns))
}
...
export function buildCandidates(s: GameState, p: Player): BuildCandidate[] {
  ...
  for (const c of all) {
    if (!queue.some((q) => q.id === c.id)) queue.push(c)
...
export type StrategyKind = 'DEVELOP' | 'MILITARIZE' | 'STRIKE' | 'FORTIFY' | 'COUP_BANK'
...
export function planFor(s: GameState, p: Player): Plan {
  ...
  if (prev && prev.computedTurn === s.turn) return prev
  const plan = computePlan(s, p, prev?.kind ?? null)
  per.set(p.id, plan)
  return plan
}

function computePlan(s: GameState, p: Player, prevKind: StrategyKind | null): Plan {
  ...
  const develop = queue.slice(0, 3).reduce((x, c) => x + c.worth * c.pComplete, 0) * 0.45 * ecoTempo

  // STRIKE — a conquest we can launch right now.
  const strike = bestAttackNow(s, p)
  const strikeScore = strike && strike.conquers ? strike.score * 1.25 * tempo : 0
  ...
  let troopsNeeded = 0
  const silos = owned(s, p).filter((pl) => pl.buildings.SILO)
  const staging = silos.length
    ? silos.reduce((a, b) =>
        rocketCap(b) > rocketCap(a) || (rocketCap(b) === rocketCap(a) && b.troops > a.troops)
          ? b
          : a,
      )
    : null
  if (!p.pacifistStatus) {
    ...
    for (const target of s.planets) {
      ...
      while (
        need < 80 &&
        battleWinProb(COMBAT.attackPerTroop * need + bonus, defB) < effMinConquerProb(s)
      )
        need++
      // An L2 silo is one upgrade away from unlimited capacity; below that the
      // rocket simply cannot carry the force.
      if (staging && rocketCap(staging) < need && (staging.buildings.SILO || 0) < 2) continue
      const have = staging ? staging.troops : 0
      const eta = Math.max(0, Math.ceil((need + W.reserveTroops - have) / rr)) + (staging ? 0 : 4)
      if (eta > W.planHorizon + 4) continue
      const hold = holdProbability(s, p, target, survivorsAfterWin(need), s.turn + CONQUEST_TRUCE)
      const value = planetValue(s, target) + (defOwner.planets.length === 1 ? 10 : 0)
      const sc = (value * 0.75 * hold * Math.pow(0.9, eta) - need * W.troopValue * 0.3) * tempo
      if (sc > militarize) {
        militarize = sc
        targetId = target.id
  ...
  if (coupTgt && s.turn >= ACTION_CARDS_FROM_TURN) {
    ...
    const turnsTo =
      p.influence >= coupCost ? 0 : starIncome > 0.05 ? (coupCost - p.influence) / starIncome : 99
    if (turnsTo <= W.planHorizon * 2) coupBank = coupTgt.value * Math.pow(0.92, turnsTo) * 0.8
    ...
    if (p.influence < coupCost) {
      if (p.planets.length <= 1) coupBank *= 0.35
  ...
  if (prevKind) scores[prevKind] *= W.planStickiness // hysteresis against thrashing
  let kind: StrategyKind = 'DEVELOP'
  for (const k of Object.keys(scores) as StrategyKind[]) if (scores[k] > scores[kind]) kind = k
  ...
  const enabler =
    kind === 'MILITARIZE' || kind === 'STRIKE'
      ...
      : kind === 'FORTIFY'
        ? (c: BuildCandidate) =>
            (c.id === 'BARRACKS' && !hasB(s, p, 'BARRACKS')) || c.id === 'SHIELD'
        : null
  if (enabler) queue = [...queue.filter(enabler), ...queue.filter((c) => !enabler(c))]
...
function skipTarget(s: GameState, p: Player, t: InfluenceType): Player | null {
  ...
  if (t === 'SKIP_ARMY')
    return rivals.reduce((a, b) => (totalTroops(s, b) > totalTroops(s, a) ? b : a))
  if (t === 'SKIP_PLANETS')
...
export function denialValue(s: GameState, p: Player, t: PoolType): number {
  ...
  for (const r of alive(s)) {
    ...
    const w = Math.min(2, Math.max(0.3, playerStrength(s, r) / Math.max(1, avg)))
    let gain = 0
    if (def.building) {
      const id = t as BuildingType
      if (id === 'SINGULARITY' && singularityReadyFor(s, r)) gain = 5 // deny the draft engine
      else if (rivalGoalBuilding(s, r)?.id === id) gain = 2.5
    } else if (def.influenceCard) {
      if (r.influence >= INFLUENCE_CARDS[t as InfluenceType].cost)
        gain = t === 'COUP' ? 6 : 1.5 // a rich rival + a Coup card is an emergency
    } else if (t === 'ATTACK') {
      if (!r.pacifistStatus && hasB(s, r, 'SILO') && aggression(r) >= W.willNeutral) gain = 1.4
    } else if (RESOURCE_TYPES.includes(t as never)) {
      const goal = rivalGoalBuilding(s, r)
      if (goal && (goal.cost[t] || 0) > (r.hand[t] || 0)) gain = 0.7
    }
...
function influenceDraftValue(s: GameState, p: Player, t: InfluenceType, plan: Plan): number {
  const cost = INFLUENCE_CARDS[t].cost
  // Opportunity cost of the stars — steep while we're saving toward a Coup.
  const starCost = cost * (plan.kind === 'COUP_BANK' && t !== 'COUP' ? 1.2 : 0.35)
  let v = 0
  if (t === 'COUP') {
    ...
    if (tgt && tgt.value >= W.coupValueFloor) return 12 - ((p.hand.COUP || 0) > 0 ? 6 : 0)
    v = -2
  } else if (t === 'STEAL_ACTION') {
    const loot = alive(s).some(
      (x) => x.id !== p.id && (['ATTACK', 'RECRUIT', 'MOVE', 'TRADE'] as ActionType[]).some((a) => (x.hand[a] || 0) > 0),
    )
    v = loot ? 1.5 : -2
  } else if (t === 'PEACE') {
    v = 1 + plan.threat * 0.4
  ...
  if ((p.hand[t] || 0) > 0) v -= 1.5 // one copy in hand is usually enough
  return v - starCost
}
...
function ownDraftValue(
  ...
  if (def.building) {
    ...
    if (!level) return -1 // pickable[] should already exclude this
    const worth = buildingWorth(s, p, id, draftPlanet, level)
    let v = 1.5 + worth / 6
    if (plan.buildQueue[0]?.id === id) v += 2
    else if (plan.buildQueue.some((c) => c.id === id)) v += 1
    ...
    if (head && head.id !== id && canAfford(p.hand, head.cost)) {
      ...
      if (!canAfford(after, head.cost)) v -= 2
    }
    return v
  }
  if (def.influenceCard) return influenceDraftValue(s, p, t as InfluenceType, plan)
  if (t === 'ATTACK') {
    if (p.pacifistStatus) return -1
    let v = 1.2
    if ((plan.kind === 'STRIKE' || plan.kind === 'MILITARIZE') && hasB(s, p, 'SILO')) v += 1.6
    if ((p.hand.ATTACK || 0) === 0 && hasB(s, p, 'SILO') && totalTroops(s, p) >= 4) v += 1
  ...
  if (t === 'RECRUIT') {
    ...
    if (hasB(s, p, 'BARRACKS') && owned(s, p).some((pl) => pl.troops < garrisonFloor(s))) v += 1.5
    v += Math.min(2.5, plan.threat * 0.4)
    if (plan.kind === 'MILITARIZE' || plan.kind === 'STRIKE') v += 1.6
    return v - (p.hand.RECRUIT || 0) * 0.4
  }
  if (t === 'MOVE') {
    let v = 0.8
    if (p.planets.length >= 2 && hasB(s, p, 'SPACEPORT')) v += 0.8
    return v - (p.hand.MOVE || 0) * 0.6
  }
  if (t === 'TRADE') {
    let v = 1
    if (hasB(s, p, 'EMBASSY')) v += 0.6
  ...
  let v = def.value
  const head = plan.buildQueue[0]
  if (head && (head.cost[t] || 0) > (p.hand[t] || 0)) v += 1.6
  if ((plan.kind === 'MILITARIZE' || plan.kind === 'STRIKE') && t === 'ORE') v += 0.8 // recruit fuel
  if (t === 'RELIC') v += 0.3 // wildcard flexibility
...
function imminentAttacker(s: GameState, us: Player, r: Player): boolean {
  ...
  for (const pl of owned(s, us)) {
    ...
    if (strike.n < minTroopsToConquer(pl.troops)) continue
    const pWin = battleWinProb(
      COMBAT.attackPerTroop * strike.n + strike.bonus,
      defenseBaseOf(s, pl),
    )
    if (pWin >= 0.35) return true
  }
  return false
}
...
export function influencePlay(
  ...
  if ((p.hand.PEACE || 0) > 0) {
    const worst = Math.max(0, ...owned(s, p).map((pl) => immediateFallProb(s, p, pl)))
    if (worst >= W.peaceThreatFloor) return { type: 'PEACE', opts: {}, ev: worst * 10 }
  }
  ...
  for (const t of ['SKIP_ARMY', 'SKIP_PLANETS', 'SKIP_TECH', 'SKIP_INFLUENCE'] as InfluenceType[]) {
    ...
    const scary =
      ...
      alive(s).length === 2
    if (scary) return { type: t, opts: {}, ev: 3 }
  }
  ...
  if ((p.hand.STEAL_ACTION || 0) > 0) {
    const rivals = alive(s).filter((x) => x.id !== p.id)
    const byStrength = (a: Player, b: Player) => playerStrength(s, b) - playerStrength(s, a)
    const danger = rivals.filter((r) => imminentAttacker(s, p, r)).sort(byStrength)
    if (danger.length)
      return { type: 'STEAL_ACTION', opts: { target: danger[0], cardType: 'ATTACK' }, ev: 3 }
    if (
      (plan.kind === 'STRIKE' || plan.kind === 'MILITARIZE') &&
      (p.hand.ATTACK || 0) === 0 &&
      hasB(s, p, 'SILO')
    ) {
      const holder = rivals.filter((r) => (r.hand.ATTACK || 0) > 0).sort(byStrength)[0]
      if (holder)
        return { type: 'STEAL_ACTION', opts: { target: holder, cardType: 'ATTACK' }, ev: 2.5 }
    }
    // Otherwise: take a card we can use from a strong rival (gain + denial).
    const wants: ActionType[] = []
    if (hasB(s, p, 'BARRACKS') && (p.hand.RECRUIT || 0) === 0) wants.push('RECRUIT')
    if (hasB(s, p, 'EMBASSY') && (p.hand.TRADE || 0) === 0) wants.push('TRADE')
    for (const a of wants) {
      const holder = rivals
        .filter((r) => (r.hand[a] || 0) > 0 && playerStrength(s, r) >= avg)
        .sort(byStrength)[0]
      if (holder) return { type: 'STEAL_ACTION', opts: { target: holder, cardType: a }, ev: 2 }
    }
  }
  return null
}
...
function desiredGarrison(s: GameState, p: Player, planet: Planet): number {
  ...
  if (p.planets.length === 1) want += 4 // the last planet has no fallback — hold it hard
  if (planet.buildings.SILO) want += 4 // launch pad wants a striking reserve
...
export function mastermindAction(s: GameState, p: Player): MastermindDecision | null {
  ...
  if (inf) return { kind: 'influence', type: inf.type, opts: inf.opts }

  // 2. Strike — the planner already checked win odds AND the 5-turn hold.
  if ((p.hand.ATTACK || 0) > 0) {
    const atk = bestAttackNow(s, p)
    if (atk) return { kind: 'attack', source: atk.source, target: atk.target, n: atk.n }
  }
  ...
  if ((p.hand.RECRUIT || 0) > 0) {
    const affordable = (pl: Planet) =>
      (pl.buildings.BARRACKS || 0) > 0 && canAfford(p.hand, { ORE: recruitYieldOf(pl) })
    ...
    if (danger.length) return { kind: 'recruit', planet: danger[0] }
    // (b) Stack the strike force whenever we hold the means to use it.
    const stacking =
      plan.kind === 'MILITARIZE' ||
      plan.kind === 'STRIKE' ||
      ((p.hand.ATTACK || 0) > 0 && plan.stagingId != null)
    const staging = plan.stagingId != null ? s.planets[plan.stagingId] : null
    if (
      stacking &&
      staging &&
      staging.ownerId === p.id &&
      staging.troops < Math.max(plan.troopsNeeded, desiredGarrison(s, p, staging))
    ) {
      if (affordable(staging)) return { kind: 'recruit', planet: staging }
      const any = pls
        .filter(affordable)
        .sort((a, b) => (b.buildings.BARRACKS || 0) - (a.buildings.BARRACKS || 0))[0]
      if (any) return { kind: 'recruit', planet: any } // move it to staging later
    }
    ...
    const thin = pls
      ...
      .sort(
        (a, b) => a.troops - desiredGarrison(s, p, a) - (b.troops - desiredGarrison(s, p, b)),
      )[0]
    if (thin) return { kind: 'recruit', planet: thin }
  }

  // 4. Move — rescue a threatened planet, or mass the strike force.
  if ((p.hand.MOVE || 0) > 0 && hasB(s, p, 'SPACEPORT') && p.planets.length >= 2) {
    ...
    for (const dest of inDanger) {
      const donor = pls
        .filter(
          (pl) => pl !== dest && pl.troops > floor + 2 && immediateFallProb(s, p, pl) < 0.2,
        )
        .sort((a, b) => b.troops - a.troops)[0]
      if (donor) {
        const n = Math.min(rocketCap(donor), donor.troops - floor)
        if (n >= 1) return { kind: 'move', from: donor, to: dest, n }
      }
    }
    ...
    if (
      ...
      if (donor) {
        const n = Math.min(rocketCap(donor), donor.troops - floor)
        if (n >= 2) return { kind: 'move', from: donor, to: staging, n }
      }
    }
  }
  ...
  if (!p.tradedThisTurn && (p.hand.TRADE || 0) > 0 && hasB(s, p, 'EMBASSY')) {
    ...
    if (offer) return { kind: 'trade', ...offer }
  }
  return null
}
...
function planTradeOffer(
  ...
  if (head)
    want = RESOURCE_TYPES.find((t) => t !== 'RELIC' && (head.cost[t] || 0) > (p.hand[t] || 0)) ?? null
  if (!want && (plan.kind === 'MILITARIZE' || plan.kind === 'STRIKE') && (p.hand.ORE || 0) < 3)
    want = 'ORE'
  if (!want) return null
  // Surplus = hand beyond what the queue head still needs (never give relics).
  const reserved: Cost = {}
  if (head) for (const t in head.cost) reserved[t] = head.cost[t]
  const surplus: string[] = []
  for (const t of RESOURCE_TYPES) {
    if (t === 'RELIC' || t === want) continue
  ...
  const partner = partners.find((x) => playerStrength(s, x) < avg * 1.3) ?? partners[0]
...
export function mastermindEvaluateTrade(
  ...
  let vOut = 0
  for (const t in gets)
    vIn += gets[t] * CARDS[t].value * (head && (head.cost[t] || 0) > (ai.hand[t] || 0) ? 1.35 : 1)
  for (const t in gives) vOut += gives[t] * CARDS[t].value
  if (head && canAfford(ai.hand, head.cost)) {
    const after: Hand = { ...ai.hand }
    for (const t in gives) after[t] = (after[t] || 0) - gives[t]
    for (const t in gets) after[t] = (after[t] || 0) + gets[t]
    if (!canAfford(after, head.cost)) vOut *= 1.6 // would un-afford our next build
  }
  // Anti-kingmaker: the leader pays a premium for our cooperation.
  if (proposer && proposer.id !== ai.id && playerStrength(s, proposer) > avgStrength(s) * 1.25) {
    return vIn >= vOut * 1.5
  }
