...
import { animateRocket, boom, floatText, sleep } from './effects';
...
const AUTO_HUMAN =
  !HAS_DOM ||
  (typeof location !== 'undefined' && /[&?]auto/.test(location.search));
...
export function canPickCard(
  ...
  if (CARDS[t].building) {
    ...
    if (bt === 'SINGULARITY' && !singularityLabOk(planet, next)) {
      return false;
    }
...
export function humanPoolClick(idx: number): void {
  if (!poolResolve || state.phase !== 'draft') {
    return;
  }
...
function aiDraftPick(p: Player, planet: Planet): number {
  ...
  if (persOf(p) === 'random') {
    ...
    for (let i = 0; i < state.pool.length; i++) {
      if (canPickCard(p, state.pool[i], planet)) {
        pickable.push(i);
      }
    }
  ...
  for (let i = 0; i < state.pool.length; i++) {
    ...
    if (CARDS[t].building) {
      ...
      if (t === 'BARRACKS' && !hasBuilding(p, 'BARRACKS')) {
        score += 2.5;
      } // Can't recruit without one
      if (t === 'LAB' && !hasBuilding(p, 'LAB')) {
        score += 1.5;
      } // Opens the road to the Singularity
      if (t === 'SILO' && !hasBuilding(p, 'SILO')) {
        score += 2;
      } // Can't attack without one
      if (t === 'SILO' && p.hand.ATTACK >= 3) {
        score += 2.5;
      } // Attacks piling up — need bigger rockets
      if (
        t === 'EMBASSY' &&
      ...
      if (
        ...
        score += 1.2;
      } // Multi-planet empires need Move
      if (t === 'SINGULARITY') {
        score += 5;
      } // Raises TECHNOLOGY and grows the draft engine
      ...
      if (it.startsWith('SKIP_')) {
        ...
        if (
          ...
          score += 1;
        } // Paralyse rivals before striking
        ...
        switch (it) {
          case 'STEAL_ACTION': {
            ...
            if (
              ...
              score += 0.8;
            } // Deny rivals their attack cards
...
function aiPickAttack(
  ...
  if (pers !== 'pacifist') {
    needMargin -=
      Math.floor(state.turn / 8) + (alivePlayers().length === 2 ? 3 : 0);
