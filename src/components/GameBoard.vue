...
function layoutBoard(): void {
  ...
  for (let i = 0; i < n; i++) {
    stars.push({ x: Math.random() * canvasW, y: Math.random() * canvasH, r: Math.random() * 1.3 + 0.3, p: Math.random() * 6.28 })
  }
}
...
function drawFrame(now: number): void {
  ...
  let g = ctx.createRadialGradient(canvasW * 0.25, canvasH * 0.3, 0, canvasW * 0.25, canvasH * 0.3, canvasW * 0.5)
  g.addColorStop(0, 'rgba(90,40,140,0.10)')
  g.addColorStop(1, 'transparent')
  ctx.fillStyle = g
  ctx.fillRect(0, 0, canvasW, canvasH)
  g = ctx.createRadialGradient(canvasW * 0.78, canvasH * 0.7, 0, canvasW * 0.78, canvasH * 0.7, canvasW * 0.45)
...
function drawPlanet(pl: Planet, now: number): void {
  ...
  const g = ctx.createRadialGradient(x - r * 0.4, y - r * 0.4, r * 0.15, x, y, r)
  ...
  if (st.feature === 'continents') {
    ...
    const blobs = [[-0.3, -0.2, 0.45], [0.28, 0.12, 0.34], [-0.05, 0.42, 0.28], [0.42, -0.36, 0.22]]
    ...
    const dots = [[-0.4, 0.2], [-0.15, 0.45], [0.15, 0.3], [0.4, 0.45], [0.05, 0.05], [-0.5, 0.5], [0.3, 0.1], [-0.25, -0.1]]
    ...
    ctx.beginPath()
    ctx.ellipse(x + r * 0.25, y - r * 0.15, r * 0.28, r * 0.16, 0.4, 0, Math.PI * 2)
    ctx.fill()
  } else if (st.feature === 'craters') {
    ctx.fillStyle = 'rgba(0,0,20,0.25)'
    for (const [dx, dy, dr] of [[-0.3, 0.1, 0.18], [0.25, -0.25, 0.14], [0.1, 0.4, 0.12]]) {
      circle(x + dx * r, y + dy * r, dr * r)
      ctx.fill()
    }
  } else if (st.feature === 'lava') {
    ctx.fillStyle = 'rgba(30,5,5,0.75)'
    for (const [dx, dy, rw, rh, ang] of [[-0.2, 0.1, 0.34, 0.18, 0.5], [0.3, -0.2, 0.24, 0.38, 1.2], [-0.05, -0.4, 0.3, 0.15, 0.8], [0.1, 0.4, 0.36, 0.2, 2.1]]) {
      ctx.beginPath()
      ctx.ellipse(x + dx * r, y + dy * r, rw * r, rh * r, ang, 0, Math.PI * 2)
      ctx.fill()
    }
    ctx.fillStyle = 'rgba(255,110,0,0.32)'
    for (const [dx, dy] of [[-0.1, 0.06], [0.24, -0.14], [-0.16, -0.32]]) {
      circle(x + dx * r, y + dy * r, 0.11 * r)
      ctx.fill()
    }
  } else if (st.feature === 'forest') {
    ctx.fillStyle = 'rgba(10,55,15,0.82)'
    for (const [dx, dy, dr] of [[-0.22, -0.08, 0.46], [0.26, 0.22, 0.38], [-0.06, 0.42, 0.34], [0.38, -0.36, 0.28], [-0.42, 0.28, 0.3]]) {
      circle(x + dx * r, y + dy * r, dr * r)
      ctx.fill()
    }
  } else if (st.feature === 'toxic') {
    ctx.fillStyle = 'rgba(40,210,30,0.55)'
    for (const [dx, dy, dr] of [[-0.28, 0.22, 0.28], [0.22, -0.14, 0.24], [0.1, 0.44, 0.2], [-0.1, -0.34, 0.26]]) {
      circle(x + dx * r, y + dy * r, dr * r)
      ctx.fill()
    }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      ctx.fillRect(x + Math.cos(a) * r * 0.32 - 0.11 * r, y + Math.sin(a) * r * 0.32 - 0.07 * r, 0.22 * r, 0.14 * r)
    }
    ...
    ctx.fill()
    ctx.beginPath()
    ctx.ellipse(x + r * 0.2, y - r * 0.3, r * 0.28, r * 0.1, -0.6, 0, Math.PI * 2)
    ctx.fill()
    ...
    ctx.beginPath()
    ctx.ellipse(x + r * 0.2, y - r * 0.18, r * 0.42, r * 0.22, -0.5, 0, Math.PI * 2)
    ctx.fill()
  } else if (st.feature === 'radiation') {
    ctx.fillStyle = 'rgba(150,250,50,0.42)'
    for (const [dx, dy, dr] of [[-0.3, 0.12, 0.2], [0.26, 0.28, 0.17], [-0.1, -0.32, 0.22], [0.38, -0.22, 0.15]]) {
      circle(x + dx * r, y + dy * r, dr * r)
      ctx.fill()
    }
  }
  ...
  for (let row = 0; row * 5 < icons.length; row++) {
    ctx.fillText(icons.slice(row * 5, row * 5 + 5).join(' '), x, y + r + 33 + row * 14)
  }
}
