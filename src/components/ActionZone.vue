...
function onRecruit(): void {
  const pls = ownedPlanets(human.value).filter((pl) => pl.buildings.BARRACKS && canAfford(human.value.hand, recruitCost(pl)))
...
<template>
  <div id="action-zone">
    ...
    <button
      class="btn action"
      :disabled="!my || !hasActionCard(human, 'RECRUIT') || !canRecruitSomewhere"
      :title="recruitTitle"
      @click="onRecruit"
    >
    ...
    <button
      class="btn action"
      :disabled="!my || !hasActionCard(human, 'MOVE') || !hasPort || human.planets.length < 2 || totalTroops(human) < 1"
      :title="moveTitle"
    ...
    <button
      class="btn action"
      :disabled="!my || !hasActionCard(human, 'TRADE') || !hasEmbassy || alivePlayers().length < 2"
      :title="tradeTitle"
    ...
    <button class="btn action end" id="btn-end" :disabled="!my" @click="store.endTurn()">⏭️ End Turn</button>
  </div>
</template>
