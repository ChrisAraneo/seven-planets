...
function coupIcons(pl: Planet): string {
  return BUILD_ORDER.filter((b) => pl.buildings[b])
    .map((b: BuildingType) => BUILDINGS[b].icon + (pl.buildings[b] > 1 ? pl.buildings[b] : ''))
    .join(' ')
}
...
function doSteal(target: Player, cardType: 'RECRUIT' | 'ATTACK' | 'MOVE' | 'TRADE'): void {
...
<template>
  <ModalShell @close="store.closeModal()">
    ...
    <template v-if="view === 'main'">
      ...
      <p class="dimtx">These cards were paid for at draft time — playing one is free. Click a card to play it.</p>
      <div v-for="t in held" :key="t" class="trow" @click="chooseCard(t)">
        <div class="tinfo">
          <b>{{ INFLUENCE_CARDS[t].icon }} {{ INFLUENCE_CARDS[t].name }}</b> ×{{ human.hand[t] }} —
          <span class="dimtx">{{ INFLUENCE_CARDS[t].desc }}</span>
          <template v-if="t.startsWith('SKIP_')">
            <span v-if="skipTarget(t)" class="dimtx">
              → would hit <b :style="{ color: skipTarget(t)!.color }">{{ skipTarget(t)!.name }}</b></span>
    ...
    <template v-else-if="view === 'coup'">
      ...
      <template v-if="coupList.length">
        <div v-for="pl in coupList" :key="pl.id" class="trow" @click="doCoup(pl)">
          <div class="tinfo">
            <b :style="{ color: store.state.players[pl.ownerId].color }">{{ pl.name }}</b> — {{ store.state.players[pl.ownerId].name }}
          </div>
    ...
    <template v-else>
      <h2>🎭 EXTORTION</h2>
      <p class="dimtx">Take one action card of your choice from a chosen rival (influence cards cannot be taken).</p>
