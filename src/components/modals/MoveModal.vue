...
const fromId = ref(ownedPlanets(human).reduce((a, b) => (a.troops >= b.troops ? a : b)).id)
const toId = ref(-1)
const n = ref(1)

const from = computed(() => store.state.planets[fromId.value])
const owned = computed(() => ownedPlanets(human))
const dests = computed(() => owned.value.filter((pl) => pl.id !== fromId.value))
const cap = computed(() => Math.min(rocketCap(from.value), from.value.troops))

watch(
  [dests, fromId],
  () => {
    if (!dests.value.some((pl) => pl.id === toId.value)) toId.value = dests.value.length ? dests.value[0].id : -1
    n.value = Math.max(1, Math.min(n.value, Math.max(1, cap.value)))
  },
  { immediate: true },
)

const capFmt = computed(() => (rocketCap(from.value) === Infinity ? '∞ (all troops)' : String(rocketCap(from.value))))
...
<template>
  <ModalShell @close="store.closeModal()">
    ...
    <p class="dimtx">
      Move an army between your planets. Uses the origin's rockets (capacity {{ capFmt }}). Spends one 🛸 Move card (you
      have {{ human.hand.MOVE }}).
    </p>
    <p>
      From:
      <button
    ...
    <div v-for="pl in dests" :key="pl.id" class="trow" :class="{ sel: pl.id === toId }" @click="toId = pl.id">
      <div class="tinfo"><b>{{ pl.name }}</b></div>
      <div>🪖{{ pl.troops }}</div>
    </div>
