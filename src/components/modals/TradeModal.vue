...
function changePartner(id: number): void {
  partnerId.value = id
  for (const t of RESOURCE_TYPES) get[t] = Math.min(get[t], partner.value.hand[t])
  note.value = { msg: 'Adjust the offer, then transmit it.', cls: 'info' }
}

function step(side: 'give' | 'get', t: string, delta: number): void {
  if (side === 'give') give[t] = Math.max(0, Math.min(human.hand[t], give[t] + delta))
...
function propose(): void {
  ...
  if (accept) {
    ...
    note.value = { msg: `${partner.value.name} scoffs at your offer.`, cls: 'warn' }
  }
}
</script>
...
<template>
  <ModalShell @close="store.closeModal()">
    ...
    <p>
      Partner:
      <select class="btn" :value="partnerId" @change="changePartner(+($event.target as HTMLSelectElement).value)">
    ...
    <table class="ttable">
      <tr><th></th><th>YOU GIVE</th><th>YOU RECEIVE</th></tr>
      <tr v-for="t in RESOURCE_TYPES" :key="t">
        <td>{{ CARDS[t].icon }} {{ CARDS[t].name }}</td>
        <td>
          <span class="stepper">
            <button @click="step('give', t, -1)">−</button><span class="sval">{{ give[t] }}</span><button @click="step('give', t, 1)">+</button>
          </span>
          <span class="dimtx">/ {{ human.hand[t] }}</span>
        </td>
        <td>
          <span class="stepper">
            <button @click="step('get', t, -1)">−</button><span class="sval">{{ get[t] }}</span><button @click="step('get', t, 1)">+</button>
          </span>
          <span class="dimtx">/ {{ partner.hand[t] }}</span>
        </td>
      </tr>
    </table>
    <div class="mnote" :class="{ warn: note.cls === 'warn' }" :style="note.cls === 'ok' ? { color: '#7dff8a' } : {}">
      {{ note.msg }}
    </div>
    <div class="mbtns">
