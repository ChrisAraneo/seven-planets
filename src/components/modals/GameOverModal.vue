...
const humanWon = computed(() => !!over.value?.winner && over.value.winner.isHuman)

const title = computed(() => {
  if (!over.value) return ''
  if (over.value.reason === 'conquest') return humanWon.value ? 'GALACTIC EMPEROR' : 'DEFEAT'
...
<template>
  <ModalShell v-if="over">
    <div class="gameover-title" :class="humanWon ? 'win' : 'lose'">{{ title }}</div>
    <div class="gostats">
      {{ sub }}<br /><br />
      Turns played: {{ store.state.turn }} · Planets held: {{ human.planets.length }} · Buildings:
      {{ buildingCount(human) }} · Troops: {{ totalTroops(human) }}
    </div>
