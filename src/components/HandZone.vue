...
import { CARD_TYPES, CARDS, INFLUENCE_CARDS, INFLUENCE_TYPES } from '@/game/constants'
    ...
    const hint =
      t === 'RELIC'
        ...
        : CARDS[t].action
          ...
          : ''
    return { t, n, icon: CARDS[t].icon, name: CARDS[t].name, color: CARDS[t].color, action: !!CARDS[t].action, title: `${CARDS[t].name}${hint}` }
  }),
)

const influence = computed(() =>
  INFLUENCE_TYPES.filter((t) => store.human.hand[t] > 0).map((t: InfluenceType) => ({
    t,
