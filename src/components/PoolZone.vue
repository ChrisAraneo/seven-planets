...
import { BUILDINGS, buildingCost, CARDS, costLabel, INFLUENCE_CARDS, maxLevel } from '@/game/constants'
...
const hints: Partial<Record<PoolType, string>> = {
  ATTACK: ' (needs a 🚀 Rocket Silo and at least 1 soldier to pick — attacks launch only from Silo planets)',
  MOVE: ' (needs a 🛰️ Spaceport and 2+ planets to pick — redeploys troops between your planets)',
  RECRUIT: ' (needs a 🎖️ Barracks to pick — recruiting is impossible without one)',
...
const poolCards = computed<PoolCardVM[]>(() => {
  ...
  return state.pool.map((t, i) => {
    ...
    if (c.building) {
      ...
      return {
        i, type: t, kind: 'building', cls: base + ' bcard', color: c.color, valid,
        icon: B.icon, name: B.name, cost, bonus: B.short, badge,
    ...
    if (c.influenceCard) {
      ...
      return {
        i, type: t, kind: 'influence', cls: base + ' bcard', color: c.color, valid,
        icon: IC.icon, name: IC.name, cost: `${IC.cost}⭐`, bonus: 'influence',
        title: `${IC.name}: ${IC.desc} — costs ${IC.cost}⭐ Influence now; goes to your hand and can be played on any of your action turns.`,
      }
    }
    return {
      i, type: t, kind: 'regular', cls: base, color: c.color, valid,
  ...
  <div id="pool-zone">
    ...
    <div id="pool">
      ...
      >
        ...
        <template v-else>
          <div class="bhead"><span class="bic2">{{ vm.icon }}</span><span class="bnm">{{ vm.name }}{{ vm.badge }}</span></div>
