...
import { evaluateAttacks } from '@/ai/functions/evaluate-attacks';
import { holdProbability } from '@/ai/functions/hold-probability';
import { mastermindDraftPick } from '@/ai/functions/mastermind-draft-pick';
import { minTroopsToConquer } from '@/ai/functions/min-troops-to-conquer';
import { planFor } from '@/ai/functions/plan-for';
...
describe('mastermind drafting', () => {
  it('picks a valid pool index and respects pickability', () => {
    ...
    const idx = mastermindDraftPick(me, s.planets[0], all);
    ...
    expect(mastermindDraftPick(me, s.planets[0], onlyTwo)).toBe(2);
  });
});
...
describe('kamikaze (Hard mode) targeting', () => {
  ...
  it('a kamikaze threatens the human but is no threat to other AI', () => {
    ...
    expect(
      holdProbability(s.players[2], s.planets[2], s.planets[2].troops),
    ).toBe(1);
    // …but the human's planet is not.
    expect(
      holdProbability(s.players[0], s.planets[0], s.planets[0].troops),
    ).toBeLessThan(1);
  });
});
