...
export type EngineCursor =
  ...
  | {
      phase: 'draft';
      /** Snapshot of draftOrder (player ids), taken when the draft starts. */
      seatQueue: number[];
      ...
      pick: number;
      /** Captured mainPicks(...) at slot entry; -1 while the slot is unentered. */
      picksTotal: number;
    }
  | {
      phase: 'action';
      /** Snapshot of turnOrder (player ids), taken when the action phase
