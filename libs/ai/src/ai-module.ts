...
/** The AI only needs the store to expose the game module's state — it stays
    unaware of the app's RootState (ui/effects/unlocks/…). Widened to
    `Store<unknown>` at the plugin boundary because Vuex `Store` is invariant
    in its state type (the app registers this via `plugins: [aiPlugin]`). */
type AiRootStore = Store<{ game: { state: GameState } }>;
...
export const ai: Module<AiModuleState, unknown> = {
  namespaced: true,
...
/* ---------------------------------------------------------------------
   ...
   same resolvers.
   --------------------------------------------------------------------- */
export function aiPlugin(storeArg: Store<unknown>): void {
  const store = storeArg as AiRootStore;
